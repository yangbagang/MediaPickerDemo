...
import com.ybg.app.cameraview.CameraView.Companion.BUTTON_STATE_BOTH
import com.ybg.app.cameraview.CameraView.Companion.BUTTON_STATE_ONLY_CAPTURE
import com.ybg.app.cameraview.CameraView.Companion.BUTTON_STATE_ONLY_RECORDER
...
class CaptureButton : View {
    ...
    private var strokeWidth: Float = 0f //进度条宽度
    ...
    private var button_radius: Float = 0f //按钮半径
    ...
    private fun handlerUnpressByState() {
        ...
        when (state) {
        //当前是点击按下
            STATE_PRESS -> if (captureListener != null && (button_state == BUTTON_STATE_ONLY_CAPTURE || button_state == BUTTON_STATE_BOTH)) {
    ...
    private fun recordEnd() {
        if (captureListener != null) {
            if (recorded_time < min_duration)
                captureListener!!.recordShort(recorded_time.toLong())//回调录制时间过短
            else
                captureListener!!.recordEnd(recorded_time.toLong())  //回调录制结束
        }
