...
import com.ybg.app.cameraview.util.AngleUtil
import com.ybg.app.cameraview.util.CameraParamUtil
import com.ybg.app.cameraview.util.CheckPermission
...
class CameraInterface private constructor() : Camera.PreviewCallback {
    ...
    private fun rotationAnimation() {
        ...
        if (rotation != angle) {
            ...
            val animC = ObjectAnimator.ofFloat(mSwitchView, "rotation", start_rotaion.toFloat(),
                    end_rotation.toFloat())
            val animF = ObjectAnimator.ofFloat(mFlashLamp, "rotation", start_rotaion.toFloat(),
                    end_rotation.toFloat())
            val set = AnimatorSet()
    ...
    fun doStartPreview(holder: SurfaceHolder?, screenProp: Float) {
        ...
        if (mCamera != null) {
            try {
                mParams = mCamera!!.parameters
                val previewSize = CameraParamUtil.instance.getPreviewSize(mParams!!
                        .supportedPreviewSizes, 1000, screenProp)
                val pictureSize = CameraParamUtil.instance.getPictureSize(mParams!!
    ...
    fun takePicture(callback: TakePictureCallback?) {
        ...
        //
        Log.i("CJT", angle.toString() + " = " + cameraAngle + " = " + nowAngle)
    ...
    fun startRecord(surface: Surface, screenProp: Float, callback: ErrorCallback) {
        ...
        videoFirstFrame = createBitmap(videoFirstFrame!!, 0, 0, videoFirstFrame!!.width, videoFirstFrame!!
                .height, matrix, true)
        ...
        if (mParams!!.supportedVideoSizes == null) {
            videoSize = CameraParamUtil.instance.getPreviewSize(mParams!!.supportedPreviewSizes, 600,
                    screenProp)
        } else {
            videoSize = CameraParamUtil.instance.getPreviewSize(mParams!!.supportedVideoSizes, 600,
                    screenProp)
        }
